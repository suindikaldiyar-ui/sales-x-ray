...
export default async function FunnelPage({
  ...
  return (
    <>
      <PageHeader
        title="Воронка"
        description={`«${data.selectedPipelineName}» · конверсия по позиционному рангу этапа`}
      />
      ...
      {!hasData ? (
        ...
        <>
          ...
          <Card className="overflow-x-auto p-0">
            <div className="min-w-[720px]">
              ...
              {r.funnel.map((s) => {
                ...
                const isBottleneck =
                  r.bottleneck && r.bottleneck.toStage === s.name;
                return (
                  <div
                    key={s.rank}
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="truncate font-medium text-content" title={s.name}>
                          {s.name}
                        </span>
                      ...
                      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-ink-700">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-xray/40 to-xray"
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
                    {/* current on stage — matches the amoCRM kanban */}
                    <span className="nums text-right text-lg font-semibold text-xray">
                      {formatNumber(s.current)}
                    </span>
                    {/* cohort reached */}
                    <span className="nums text-right font-medium text-content-muted">
          ...
          <p className="mt-3 text-xs text-content-faint">
            «Сейчас на этапе» — сколько сделок стоит на этапе прямо сейчас
            (как в канбане amoCRM). «Дошло» — сколько сделок когда-либо
            достигали этапа (когортно); по нему считается конверсия и узкое
            место.
          </p>

          {/* won / lost summary so the column sums reconcile with amoCRM */}
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            ...
            <div className="panel-flat p-4">
              ...
              <p className="nums mt-1 font-display text-2xl font-bold text-signal-good">
                {formatNumber(r.wonCount)}
              </p>
            </div>
            <div className="panel-flat p-4">
              <p className="text-xs text-content-faint">Закрыто / слив (143)</p>
              <p className="nums mt-1 font-display text-2xl font-bold text-signal-bad">
                {formatNumber(r.lostCount)}
              </p>
            </div>
          </div>
                  ...
                  {r.lossReasons.slice(0, 8).map((reason) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        <span className="nums text-content-muted">
                          {formatNumber(reason.count)} шт
                        </span>
                        <span className="nums w-24 text-right text-content-faint">
                          {fmtMoney(reason.value)}
                        </span>
                      </div>
                    </div>
                  ))}
