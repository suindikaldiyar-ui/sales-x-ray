...
import { ManagersSkeleton } from "@/components/app/report-skeleton";
  ...
  return (
    <>
      <p className="mb-4 text-sm text-content-muted">
        {formatNumber(data.totalLeadsInPeriod)} сделок · {managers.length}{" "}
        {managers.length === 1 ? "менеджер" : "менеджеров"}
      </p>
          ...
          <tbody>
            {managers.map((m) => (
              <tr key={m.id} className="border-b border-line align-top last:border-0">
                ...
                <td className="nums px-3 py-3 text-right font-medium text-content">{formatNumber(m.leads)}</td>
                <td className="nums px-3 py-3 text-right text-signal-good">{formatNumber(m.won)}</td>
                <td className="nums px-3 py-3 text-right text-signal-bad">{formatNumber(m.lost)}</td>
                <td className="nums px-3 py-3 text-right text-content-muted">{formatNumber(m.open)}</td>
                <td className="px-3 py-3 text-right">
                  <Badge tone={m.conversion >= 20 ? "good" : m.conversion >= 8 ? "warn" : "neutral"}>
                    {m.conversion}%
                  </Badge>
                </td>
                <td className="nums px-5 py-3 text-right text-content">{fmtMoney(m.wonValue)}</td>
              </tr>
            ))}
          </tbody>
        </table>
