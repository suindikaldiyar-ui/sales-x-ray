-- ============================================================================
-- Sales X-Ray — 0008 report RPCs + indexes
-- Moves dashboard/funnel/managers aggregation INTO Postgres so Node receives
-- small ready-made results instead of reading ~16k leads per render. The logic
-- is unchanged (positional rank from amocrm_stages, reached_rank, system
-- statuses 142/143, period by created_at_src, stage matching stays in TS over
-- the tiny per-stage output). All functions are SECURITY DEFINER with an
-- explicit organization-membership guard — RLS isolation is preserved.
-- ============================================================================

-- ─── indexes for the aggregate queries ─────────────────────────────────────
create index if not exists leads_org_status_idx
  on public.leads (organization_id, status_external_id);
create index if not exists leads_org_reached_idx
  on public.leads (organization_id, reached_rank);
create index if not exists leads_org_responsible_idx
  on public.leads (organization_id, responsible);
create index if not exists leads_org_pipeline_created_idx
  on public.leads (organization_id, pipeline_external_id, created_at_src);
-- (leads_org_created_idx and leads_org_pipeline_idx already exist from 0004.)

-- ─── pipelines with lead counts in the period (for the picker + default) ───
create or replace function public.report_pipelines(p_org uuid, p_days int)
...
begin
  ...
  return query
    ...
    left join public.leads l
     ...
     and (p_days is null or l.created_at_src >= now() - make_interval(days => p_days))
    where p.organization_id = p_org
    group by p.external_id, p.name, p.is_main, p.sort
    order by p.sort;
end $$;

-- ─── headline aggregates for a pipeline (won = 142, lost = 143) ────────────
create or replace function public.report_headline(p_org uuid, p_pipeline bigint, p_days int)
returns table(
...
begin
  ...
  return query
    ...
    where organization_id = p_org and source = 'amocrm'
      ...
      and (p_days is null or created_at_src >= now() - make_interval(days => p_days));
end $$;

-- ─── per-stage funnel: current (kanban) + cohort reached histogram ─────────
-- current_count = open leads whose current status maps to this open stage.
-- reached_exact = leads whose effective furthest rank == this rank (capped to
--   the pipeline's open-stage count). Node turns reached_exact into the
--   cumulative "reached" via a suffix sum, then derives conversion/bottleneck.
create or replace function public.report_funnel(p_org uuid, p_pipeline bigint, p_days int)
returns table(
...
-- ─── loss reasons for a pipeline ───────────────────────────────────────────
create or replace function public.report_loss_reasons(p_org uuid, p_pipeline bigint, p_days int)
...
-- ─── managers across ALL pipelines in the period (name from amocrm_users) ──
create or replace function public.report_managers(p_org uuid, p_days int)
returns table(
...
-- ─── per-manager current-stage distribution (small: managers × stages) ─────
create or replace function public.report_manager_stages(p_org uuid, p_days int)
...
-- ─── allow the API roles to call the RPCs ──────────────────────────────────
grant execute on function public.report_pipelines(uuid, int) to authenticated, anon;
grant execute on function public.report_headline(uuid, bigint, int) to authenticated, anon;
grant execute on function public.report_funnel(uuid, bigint, int) to authenticated, anon;
grant execute on function public.report_loss_reasons(uuid, bigint, int) to authenticated, anon;
grant execute on function public.report_managers(uuid, int) to authenticated, anon;
grant execute on function public.report_manager_stages(uuid, int) to authenticated, anon;
